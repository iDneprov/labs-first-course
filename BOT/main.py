...
if txt == "": with
open('DATA.pickle', 'wb') as f: pickle.dump(users_dict, f)
else:
with open('DATA.pickle', 'rb') as f: users_dict = pickle.load(f)
for user in users_dict.keys():
    for argument in users_dict[user].keys():
        if argument == "ExecuteStatus":
            users_dict[user][argument] = False
        elif argument == "Cash":
            users_dict[user][argument] = ""
        elif argument == "InQueueStatus":
            users_dict[user][argument] = False
        elif argument == "Homerun":
            users_dict[user][argument] = "New"
        elif argument == "Location":
            if users_dict[user]["Admin"] == True:
                users_dict[user][argument] = "Admin_Main"
            else:
                users_dict[user][argument] = "Main"
...
def start_registration(USER_ID):
    ...
    if users_dict[USER_ID]["Name"] == "":
        ...
        # ПЕРЕПИСАТЬ
        initialize_menu(USER_ID, "Пришли свою фамилию. Когда будешь готов, жми на кнопку подтвердить",
                        users_dict[USER_ID]["Location"])
        ...
        if (users_dict[USER_ID]["Name"] + "_" + str(USER_ID)) in QUEUE:
            ...
            if status == "Вызов":
                ...
                initialize_menu(USER_ID, compile_user_tag(USER_ID) + "\n"
                                                                     "Теперь твоя очередь! Защити Лабу! GO! GO! GO!",
                                users_dict[USER_ID]["Location"])
...
def change_name(USER_ID):
    if users_dict[USER_ID]["Name"]:
        ...
        # ПЕРЕПИСАТЬ
        initialize_menu(USER_ID, "Пришли новое имя. Когда будешь готов, жми на кнопку подтвердить",
                        users_dict[USER_ID]["Location"])
...
def send_queue_status(target_USER_ID):
    ...
    if target_index == 0:
        Bot.send_message(target_USER_ID, "Готовься. Ты следующий!")
    else:
        Bot.send_message(target_USER_ID, "Уже ОЧЕНЬ скоро!\nПеред тобой " + str(target_index) + " человек!")
...
def func(message):
    ...
    if message.text == "/report_bug":
        fix_menu(USER_ID, USER_LOC)
    else:
        if users_dict[USER_ID]["ExecuteStatus"] == True:
            if (message.text == "Подтвердить") and (USER_LOC == "Registration_Cab"):
                if users_dict[USER_ID]["Cash"] == "":
                    Bot.send_message(USER_ID, text="ОБЯЗАТЕЛЬНО пришли мне свою фамилию!")
                # !ВНИМАНИЕ!  -->  переписать условное выражение покороче
                elif (len(users_dict[USER_ID]["Cash"].split()) != 1) or (
                        len(users_dict[USER_ID]["Cash"].split("_")) != 1) or users_dict[USER_ID][
                    "Cash"] in BANNED_NAMES:
            ...
            if (message.text == "Подтвердить") and (USER_LOC == "Passport_Office"):
                if users_dict[USER_ID]["Cash"] == "":
                    Bot.send_message(USER_ID, text="Кто из нас хочет сменить имя? Пришли хоть что-то!")
                # !ВНИМАНИЕ!  -->  переписать условное выражение покороче
                elif (len(users_dict[USER_ID]["Cash"].split()) != 1) or (
                        len(users_dict[USER_ID]["Cash"].split("_")) != 1) or users_dict[USER_ID][
                    "Cash"] in BANNED_NAMES:
                    ...
                    users_dict[USER_ID]["Location"] = "Main"
                    initialize_menu(message, "Теперь ты " + users_dict[USER_ID]["Name"] + ". Наслаждайся",
                                    users_dict[USER_ID]["Location"])
                    users_dict[USER_ID]["NameChangeStatus"] = False
            ...
            if users_dict[USER_ID]["Admin"] == True:
                if (message.text == "Следующий") and (USER_LOC == "Admin_Main"):
                    if QUEUE:
                        ...
                        if "@" in target_tag:
                            Bot.send_message(USER_ID, text="На очереди " + target_name + "\n" + target_tag)
                        else:
                            Bot.send_message(USER_ID, text="На очереди " + target_name)
                        ...
                        Bot.send_message(USER_ID, text="Похоже в очереди СОВСЕМ пусто...")

                elif (message.text == "Отладка") and (USER_LOC == "Admin_Main"):
                    Bot.send_message(USER_ID, text=compile_debug())
                elif (message.text == "Посмотреть очередь") and (USER_LOC == "Admin_Main"):
                    if QUEUE:
                        Bot.send_message(USER_ID, text=make_queue(QUEUE))
                    elif users_dict[USER_ID]["Admin"] == True:
                        Bot.send_message(USER_ID, text="Похоже в очереди пусто...")
                ...
                elif users_dict[USER_ID]["InQueueStatus"] == True:
                    Bot.send_message(USER_ID, text="Ты УЖЕ в очереди!")
                ...
                Bot.send_photo(USER_ID,
                               open(r"C:\Users\heave\PycharmProjects\pythonProject1\media\Lain_new_year.jpg", "rb"))
                ...
                initialize_menu(message, "Вы в режиме администратора!\n"
                                         ...
                                         "для выхода из режима администрирования введите слово входа наоборот",
                                users_dict[USER_ID]["Location"])

            elif (message.text == "IVALTEK_NEW") and (USER_LOC == "Main") and (
                    users_dict[USER_ID]["InQueueStatus"] == False) and (users_dict[USER_ID]["Admin"] == False):
                ...
                Bot.send_photo(USER_ID,
                               open(r"C:\Users\heave\PycharmProjects\pythonProject1\media\Lain_sunset.jpg", "rb"))
                Bot.send_audio(USER_ID,
                               open(r"C:\Users\heave\PycharmProjects\pythonProject1\audio\IVALTEK_OP_VER.mp3", "rb"))
