...
int FindNewJ(int i,int j, int l,int k) {
    new_j = (((i+j+l)*(k+3)%25)*(((i+j+l)*(k+3)%25)>=(i*j*l*(k+4)%25)))+(i*j*l*(k+4)%25)*((i*j*l*(k+4)%25)>((i+j+l)*(k+3)%25))+10;
    return new_j;
}

int FindNewL(int i,int j, int l,int k) {
    new_l = 2*((l>0)-(l<0))*((((i+j+l)*(k+5)%10) > (i*j*l*(k+6)%25))-(((i+j+l)*(k+5)%10) < (i*j*l*(k+6)%25)))*((i+j+l)*(k+5)%10 - i*j*l*(k+6)%25);
    return new_l;
}
